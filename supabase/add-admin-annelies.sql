...
declare
  r        record;
  old_lit  text := '''robertb1023@me.com''::text';
  new_lit  text := 'any (array[''robertb1023@me.com''::text, ''annelies@reelintel.ai''::text])';
  new_qual text;
  new_chk  text;
  stmt     text;
  n        integer := 0;
begin
  ...
  loop
    new_qual := replace(coalesce(r.qual, ''),       old_lit, new_lit);
    new_chk  := replace(coalesce(r.with_check, ''), old_lit, new_lit);

    stmt := format('drop policy if exists %I on %I.%I;',
